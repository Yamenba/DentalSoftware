 ...
 * @author Ricardo
 */
public class DetalleCitaController implements Initializable {
    ...
    void initData(int id_cita){
        citas = query.citaPorId(id_cita);
        c = citas.get(0);
        p = c.getIdpaciente();
        
        Calendar fecha = new GregorianCalendar(c.getAno(),c.getMes(),c.getDia());
        SimpleDateFormat sdf = new SimpleDateFormat("dd/MMM/yyyy");

        ////////mostrar información
        label_paciente.setText(p.getNombre());
        label_fecha.setText(sdf.format(fecha.getTime()));
        label_hora.setText(horaString(c.getHoraInicio(),c.getMinutoInicio()));
        label_hora_fin.setText(horaString(c.getHoraFin(),c.getMinutoFin()));
        label_motivo.setText(c.getDescription());
        textfield_tratamiento.setText(c.getTratamientoDeLaSesion());
        imageView_fotoPerfil.setImage(new Image(getClass().getResourceAsStream("imagen_no_disponible.jpg")));
        try{
            imageView_fotoPerfil.setImage(new Image(new File(c.getIdpaciente().getRutaImagen()).toURI().toString()));    
        }catch(Exception ex){
    ...
    public void actionBotonGuardar(ActionEvent e) throws IOException{
            c.setTratamientoDeLaSesion(textfield_tratamiento.getText());
            query.saveCita(c);
            AnchorPane page = (AnchorPane) FXMLLoader.load(getClass().getResource("inicio.fxml"));
            pane.getChildren().clear();///name of pane where you want to put the fxml.
            pane.getChildren().add(page);
   ...
   @FXML
    public void actionBotonHistoriaClinica(ActionEvent e){
            ...
            try {
                ...
                controller.initData(p.getIdpaciente(),new FXMLLoader(getClass().getResource("detalleCita.fxml")),"cita",c.getIdcita());
